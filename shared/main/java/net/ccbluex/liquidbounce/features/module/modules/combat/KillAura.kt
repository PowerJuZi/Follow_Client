...
import com.sun.jna.platform.unix.X11.Button1
import java.awt.*
...
class KillAura : Module() {
    ...
    // Modes
    private val rotations = ListValue("RotationMode", arrayOf("Vanilla", "Other", "BackTrack", "Test", "LiquidbouncePlus", "Grim"), "BackTrack")
    ...
    private val AutoDisabledBlockAfterHurttime = BoolValue("AutoDisabledBlockAfterHurttime",false)
    private val autoBlock = ListValue("AutoBlockMode", arrayOf("Mouse","GameSetting","UseItem","NormalPacket","Real"),"Mouse")
    private val delayedBlockValue = BoolValue("DelayedBlock", true)
    private val LegitAutoblock = BoolValue("JuZiLegitBlock",false)
    private val AutoDisabledBlockAfterHurttimeMode = ListValue("AutoDisabledBlockAfterHurttimeMode", arrayOf("isHurttime","Ticking","isHurttimeTicking","null"),"null")
    private val blockRate = IntegerValue("BlockRate", 100, 1, 100)
    private val HighMinecraftFullBlock = BoolValue("HighMinecraftFullBlock",false)
    // Raycast
    ...
    /**
     * SuperDisabler kill aura module
     */
    ...
    private fun updateRotations(entity: IEntity): Boolean {
        ...
        if (rotations.get().equals("text", ignoreCase = true)){
            if (maxTurnSpeed.get() <= 0F)
                return true
        ...
        if (rotations.get().equals("LiquidbouncePlus", ignoreCase = true)) {
            if (maxTurnSpeed.get() <= 0F)
                return true
    ...
    private fun updateHitable() {
        // SuperDisabler hitable check if turn speed is zero
        if (maxTurnSpeed.get() <= 0F) {
            hitable = true
            return
        }
     ...
     * Start blocking
     */
    var stopblocktiking = 0
    var cancelc03 = false
    var cancelc03ticks = 0
    @EventTarget
    ...
    private fun startBlocking(interactEntity: IEntity) {
        if (AutoDisabledBlockAfterHurttimeMode.get().equals("isHurttime",true)){
            if (mc.thePlayer?.isBlocking == true){
                if (Minecraft.getMinecraft().player.hurtTime == 0){
        ...
        if (AutoDisabledBlockAfterHurttimeMode.get().equals("Ticking",true)){
            var ticks = 0
            if (mc.thePlayer?.isBlocking  == true){
                ticks++
                if (ticks == 3){
        ...
        if (AutoDisabledBlockAfterHurttimeMode.get().equals("isHurttimeTicking",true)){
            var hurttimeticking = 0
            ...
            if (mc.thePlayer?.isBlocking == true){
                ...
                if (hurttimeticking == 3){
                    mc.gameSettings.keyBindUseItem.pressed = false
                }
            }
        }
        if (AutoDisabledBlockAfterHurttimeMode.get().equals("null", true)){

        }



        if (HighMinecraftFullBlock.get()){
            if (mc.thePlayer?.isBlocking == true){
                cancelc03ticks++
        ...
        if (AutoDisabledBlockAfterHurttime.get()){
            stopblocktiking ++
            if (stopblocktiking == 2.4.toInt()){
        ...
        if(autoBlock.get().equals("Real",true)) {
            mc.netHandler.addToSendQueue(classProvider.createCPacketTryUseItem(WEnumHand.MAIN_HAND))
            blockingStatus = true
        }
        if(autoBlock.get().equals("UseItem", true)) {
            mc.gameSettings.keyBindUseItem.pressed = true
        }
        if(autoBlock.get().equals("GameSetting", true)) {
            mc.gameSettings.keyBindUseItem.pressed = true
        }
        if(autoBlock.get().equals("Mouse", true)) {
            robot.mousePress(java.awt.event.InputEvent.BUTTON3_DOWN_MASK)
        }
        if(autoBlock.get().equals("NormalPacket", true)) {
