...
@ModuleInfo(name = "GrimFull", description = "GrimFull", category = ModuleCategory.HYT)
class GrimFull : Module() {
    ...
    fun onPacket(event: PacketEvent) {
        ...
        when(modeValue.get()){
            ...
            "New" -> {
                if (packet1 is SPacketPlayerAbilities) {
                    event.cancelEvent()
                }
        ...
        if (packet1 is SPacketPlayerPosLook){
            if (disable.get()){
                val velocity = LiquidBounce.moduleManager.getModule(GrimFull::class.java) as GrimFull
    ...
    private fun closeblink() {
        try {
            ...
            while (!packets.isEmpty()) {
                Minecraft.getMinecraft().connection!!.networkManager.sendPacket(packets.take())
            }
            while (!inBus.isEmpty()) {
                inBus.poll()?.processPacket(Minecraft.getMinecraft()!!.connection)
            }
            disableLogger = false
        } catch (e: Exception) {
            e.printStackTrace()
