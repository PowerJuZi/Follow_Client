...
class Scaffold1 : Module() {
    ...
    private val placeModeValue = ListValue("PlaceTiming", arrayOf("All","Pre", "Post"), "Post")
    ...
    private val reduceSpeedInAirValue = BoolValue("CancelMoveInAir", true)
    private val moveInAirdelayValue = IntegerValue("MoveInAirDelay", 0, 0, 2000)
    ...
    private val parkour = BoolValue("Parkour", false)
    // Visuals
    ...
    private val moveInAirDelay = MSTimer()
    ...
    private fun onUpdate(event: UpdateEvent) {
        val player = mc.thePlayer ?: return
        if (parkour.get()&&MovementUtils.isMoving && player.onGround && !player.sneaking && !mc.gameSettings.keyBindSneak.isKeyDown && !mc.gameSettings.keyBindJump.isKeyDown &&
...
if (MovementUtils.isMoving) {
    if (reduceSpeedInAirValue.get() && !player.onGround && moveInAirDelay.hasTimePassed(moveInAirdelayValue.get().toLong())) {
        ...
        shouldGoDown =
            downValue.get() && !sameYValue.get() && GameSettings.isKeyDown(Minecraft.getMinecraft().gameSettings.keyBindSneak) && blocksAmount > 1
        if (shouldGoDown) {
            mc.gameSettings.keyBindSneak.pressed = false
        }
        if (slowValue.get()) {
        ...
        if (!eagleValue.get().equals("Off", true) && !shouldGoDown) {
            ...
            if (edgeDistanceValue.get() > 0) {
                for (facingType in EnumFacingType.values()) {
                    if (facingType == EnumFacingType.UP || facingType == EnumFacingType.DOWN) {
                        continue
                    }
                    val side = classProvider.getEnumFacing(facingType)
                    val neighbor = blockPos.offset(side)
                    if (isReplaceable(neighbor)) {
                        val calcDif = (if (facingType == EnumFacingType.UP || facingType == EnumFacingType.SOUTH) {
        ...
        if (player.onGround) {
            ...
            when (zitterMode.get().toLowerCase()) {
                ...
                "smooth" -> {
                    if (!GameSettings.isKeyDown(Minecraft.getMinecraft().gameSettings.keyBindRight)) {
                        mc.gameSettings.keyBindRight.pressed = false
                    }
                    if (!GameSettings.isKeyDown(Minecraft.getMinecraft().gameSettings.keyBindLeft)) {
                        mc.gameSettings.keyBindLeft.pressed = false
                    }
            ...
            while (xSearch <= if (auto) 0.9 else 0.5 + xzRV / 2) {
                ...
                while (ySearch <= if (auto) 0.9 else 0.5 + yRV / 2) {
                    ...
                    while (zSearch <= if (auto) 0.9 else 0.5 + xzRV / 2) {
                        ...
                        if (facingType != EnumFacingType.UP && facingType != EnumFacingType.DOWN) {
                            val diff = abs(if (facingType == EnumFacingType.NORTH || facingType == EnumFacingType.SOUTH) diffZ else diffX)
